...
$orders_query=mysqli_query($conn, "SELECT order_id FROM orders where seller_login_id='$_SESSION[login_user]'");
...
while ($row = mysqli_fetch_row($product_query))
{
    ...
    foreach ($row as $value)
    {
        ...
        if($count==9 && !$isPacked)
        {
            ...
            echo ''    
            . '<form class="form-inline" method="post" action="packitem.php?order_id='.$order_id.'">'                
            . '<center><button type="submit" class="btn btn-block"
                style="border-color: cyan; color: white; background-color: darkcyan">'
        ...
        {            
            ...
            . '<center><div style="font-size: 16px; padding-top: 7px; padding-bottom: 7px; '
                    . 'color: white; background-color: darkcyan">'
                .' Shipped Product - It will soon be delivered.'
            . '</div></center>';
        }
