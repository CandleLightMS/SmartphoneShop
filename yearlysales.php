...
$query = mysqli_query($conn, "SELECT * FROM orders WHERE YEAR(order_timestamp) = '$thisYear'");
...
if(mysqli_num_rows($query)>0)
{
    ...
    while($order = mysqli_fetch_row($query))
    {
        ...
        $repeat_query = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM orders WHERE YEAR(order_timestamp) = '$thisYear' "
                . "AND sp_id = $sp_id"));
        
        if($repeat_query > 1)
        {
            $minrowobj = mysqli_query($conn, "SELECT MIN(order_id) as min FROM orders WHERE YEAR(order_timestamp) = '$thisYear'"
                    . " AND sp_id = $sp_id");            
                ...
                else
                {
                    ...
                    else if($count == 10)
                    {
                        ...
                        $quantity_sold = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM orders WHERE "
                                . "YEAR(order_timestamp) = '$thisYear' AND sp_id = $sp_id"));
                        ...
                        $buyers_query = mysqli_query($conn, "SELECT customer_login_id FROM orders "
                                        . "WHERE YEAR(order_timestamp) = '$thisYear' AND sp_id = $sp_id");
