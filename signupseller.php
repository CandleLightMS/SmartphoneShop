...
try
{
    if(!empty($_POST['name'] && $_POST['address'] && $_POST['pincode'] && $_POST['phone'] && $_POST['lid'] && $_POST['email'] && $_POST['spwd']))
    {
        ...
        $email=$_POST['email'];
        $slquery = "SELECT * FROM seller WHERE s_email = '$email'";
    ...
    {        
        ...
        if($flag==0)
        {
            $sql_login = "INSERT INTO login (login_id, password, user_type) VALUES ('$_POST[lid]', '$_POST[spwd]', 'S')";
        
            if($conn->query($sql_login) == TRUE)
            {
                $sql = "INSERT INTO seller (s_name, s_address, s_pincode, s_phone, login_id, s_email)"
