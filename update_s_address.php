...
$add_query = mysqli_query($conn, "SELECT s_address FROM seller WHERE login_id = '$user_id'");
$add_array = mysqli_fetch_row($add_query);
$add = $add_array[0];

$pin_query = mysqli_query($conn, "SELECT s_pincode FROM seller WHERE login_id = '$user_id'");
$pin_array = mysqli_fetch_row($pin_query);
$pin = $pin_array[0];

$phn_query = mysqli_query($conn, "SELECT s_phone FROM seller WHERE login_id = '$user_id'");
$phn_array = mysqli_fetch_row($phn_query);
$phn = $phn_array[0];

if(isset($_POST['submitbtn']))
{
    if(!empty($_POST['address'] || $_POST['pincode']))
    {
        $address = $_POST['address'];
        $pincode = $_POST['pincode'];
        $phone = $_POST['phone'];
    
        mysqli_query($conn, "UPDATE seller SET s_address = '$address' WHERE login_id = '$user_id'");
        mysqli_query($conn, "UPDATE seller SET s_pincode = $pincode WHERE login_id = '$user_id'");
        mysqli_query($conn, "UPDATE seller SET s_phone = $phone WHERE login_id = '$user_id'");
    }
