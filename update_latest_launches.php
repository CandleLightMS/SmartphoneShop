...
while ($row = mysqli_fetch_row($sql_query))
{
    ...
    $product_query = mysqli_query($conn, "SELECT * FROM latest_launches WHERE sp_id = $row[0]");
    ...
    foreach ($row as $value)
    {
        ...
        if($count==9)
        {
            ...
            {
                echo ''    
                . '<form class="form-inline" method="post" action="latestlaunches.php?sp_id='.$row[0].'">'                
            ...
            {
                echo ''    
                . '<form class="form-inline" method="post" action="removefrom_latestlaunches.php?sp_id='.$row[0].'">'                
                . '<center><button type="submit" class="btn btn-block"
                    style="background-color: lightgray; border-color: white; color: black; border-width: 2px; font-size: 15px">'
                .' Remove from Latest Launches'
                . '</button></center>'
                . '</form>';
            }
        }    
