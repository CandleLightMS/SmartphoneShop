...
$query="SELECT * FROM smartphone where login_id='$_SESSION[login_user]'";
...
while ($row = mysqli_fetch_row($sql_query))
{
    ...
    foreach ($row as $value)
    {
        ...
        if($count==9)
        {
            ...
            echo ''    
            . '<form class="form-inline" method="post" action="update_product.php?sp_id='.$row[0].'">'                
            . '<center><button type="submit" class="btn btn-group btn-block" style="border-color: lightgray">'
                .' Update'
            . '</button></center>               '
            . '</form>';
        }    
